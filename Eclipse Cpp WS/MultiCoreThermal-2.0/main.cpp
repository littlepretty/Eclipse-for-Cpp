 ...
 * 2.0新特性：将功耗文件读入、计算结果输出还有一些其他对比稳定值的函数写入了Data.h文件中
...
int main()
{
	...
	do
	{
		...
		*/
		...
		//std::cout<<std::setw(SetWidth)<<std::setprecision(SetPrecision)<<Vbase<<"\t";
		//double compareVbase=ChargeLine(SumPower,Rbase,c_conduct,IterNumber++);
		//std::cout<<std::setw(SetWidth)<<std::setprecision(SetPrecision)<<compareVbase<<"\n";
		/*
	...
	double VbaseStable=StableVbase(CorePower,Rbase);
	Bout<<std::setw(SetWidth)<<std::right<<"Vstable: "<<VbaseStable<<"\n";
	Bout.close();
